...
class Disk(Problem):
    ...
    def successor(self, state):
        ...
        for disk in lenta:
            if disk!=0 and index+1<self.l and lenta[index+1]==0:
                newlist=list(lenta)
                newlist[index],newlist[index+1]=newlist[index+1],newlist[index]
                successor[f"D1: Disk {disk}"]=tuple(newlist)

            if disk!=0 and index+2<self.l and lenta[index+2]==0 and lenta[index+1]!=0:
                newlist=list(lenta)
                newlist[index],newlist[index+2]=newlist[index+2],newlist[index]
                successor[f"D2: Disk {disk}"]=tuple(newlist)

            if disk!=0 and index-1>=0 and lenta[index-1]==0:
                newlist=list(lenta)
                newlist[index],newlist[index-1]=newlist[index-1],newlist[index]
                successor[f"L1: Disk {disk}"]=tuple(newlist)

            if disk!=0 and index-2>=0 and lenta[index-2]==0 and lenta[index-1]!=0:
                newlist=list(lenta)
                newlist[index],newlist[index-2]=newlist[index-2],newlist[index]
                successor[f"L2: Disk {disk}"]=tuple(newlist)
            index+=1
...
if __name__ == "__main__":
    ...
    Lenta.reverse()
    problem = Disk(tuple(Prenta),L, tuple(Lenta))
    print(breadth_first_graph_search(problem).solution())



    class Disk(Problem):
        def __init__(self, n, l, initial, goal):
            super().__init__(initial, goal)
            self.goal = goal
            self.n = n
            self.l = l
        ...
        def successor(self, state):
            ...
            disks = state
            for disk in disks:
                if disk != 0 and index + 1 < self.l and disks[index + 1] == 0:
                    new = list(disks)
                    new[index], new[index + 1] = new[index + 1], new[index]
                    successors[f"D1: Disk {disk}"] = tuple(new)
                if disk != 0 and index + 2 < self.l and disks[index + 1] != 0 and disks[index + 2] == 0:
                    new = list(disks)
                    new[index], new[index + 2] = new[index + 2], new[index]
                    successors[f"D2: Disk {disk}"] = tuple(new)
                if disk != 0 and index - 1 >= 0 and disks[index - 1] == 0:
                    new = list(disks)
                    new[index], new[index - 1] = new[index - 1], new[index]
                    successors[f"L1: Disk {disk}"] = tuple(new)
                if disk != 0 and index - 2 >= 0 and disks[index - 1] != 0 and disks[index - 2] == 0:
                    new = list(disks)
                    new[index], new[index - 2] = new[index - 2], new[index]
                    successors[f"L2: Disk {disk}"] = tuple(new)
                index += 1
            return successors
    ...
    if __name__ == "__main__":
        ...
        diskovi.reverse()
        goal = tuple(diskovi)
        lenta = Disk(n, l, init, goal)
        print(breadth_first_graph_search(lenta).solution())