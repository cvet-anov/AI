...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        return state not in walls and state[0]<n and state[1] < n


    def successor(self, state):
        successors = dict()
        ghostx,ghosty= state
        if self.check_valid((state[0],state[1]+1),self.walls,self.n):
            successors["Gore 1"] = (ghostx, ghosty + 1)
        if self.check_valid((state[0], state[1] + 2), self.walls, self.n):
            successors["Gore 2"] = (ghostx, ghosty + 2)
        if self.check_valid((state[0],state[1]+3),self.walls,self.n):
            successors["Gore 3"]=(ghostx,ghosty+3)

        if self.check_valid((state[0]+1,state[1]),self.walls,self.n):
            successors["Desno 1"] = (ghostx+1, ghosty)
        if self.check_valid((state[0]+2,state[1]),self.walls,self.n):
            successors["Desno 2"] = (ghostx+2, ghosty)
        if self.check_valid((state[0]+3,state[1]),self.walls,self.n):
            successors["Desno 3"] = (ghostx+3, ghosty)

        return successors

    def h(self, node):
        return (abs(node.state[0]-(n-1))+abs(node.state[1]-(n-1)))/3
